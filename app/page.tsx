...
export default function Home() {
  ...
  const startCategory = (categoryId: string) => {
    const categoryQuestions = prepareCategoryQuestions(
  ...
  const startAllCategories = () => {
    const allQuestions = buildShuffledCategoryQuestions();
  ...
  const checkAnswer = () => {
    ...
    const correct = normalizeAnswer(userAnswer) === normalizeAnswer(currentQuestions[currentQuestionIndex].answer);
          ...
          <div className="text-center mb-12">
            <div className="mb-6">
              <div className="w-48 h-48 mx-auto mb-6 relative">
                ...
                <div className="w-full h-full bg-gradient-to-br from-slate-100 via-blue-50 to-slate-200 rounded-full shadow-2xl border-4" style={{borderColor: '#38bdf8', boxShadow: '0 0 20px rgba(56,189,248,0.4)'}} >
                  {/* Penguin Body */}
              ...
              <p className="text-xl mb-2 font-mono" style={{color: '#38bdf8'}}>{`// TERMINAL MASTERY THROUGH BRUTE FORCE REPETITION //`}</p>
              <p className="text-lg" style={{color: '#38bdf8'}}>v3.0.0 — MEMORY TECHNIQUES → OUTPUT EXAMPLES → ENHANCED AVATAR</p>
            </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {categories.map((category) => (
              <button
                ...
                style={{background: '#111827', border: '2px solid #38bdf8', boxShadow: '0 0 10px rgba(56,189,248,0.2)'}}
              >
                <div className="text-left">
                  <h3 className="text-lg font-semibold mb-2" style={{color: '#38bdf8'}}>{category.name}</h3>
                  <p className="text-sm mb-3" style={{color: '#38bdf8'}}>{category.count} questions</p>
                  <div style={{color: '#38bdf8', fontWeight: 'bold'}}>▶ START DRILL</div>
                </div>
              </button>
            ))}
          </div>
  ...
  if (!currentQuestion) {
    return (
      <div className="min-h-screen p-8 flex items-center justify-center" style={{background: '#111827'}}>
        <div className="rounded-xl shadow-lg p-8 text-center max-w-md" style={{border: '2px solid #38bdf8', boxShadow: '0 0 20px rgba(56,189,248,0.3)', background: '#111827'}}>
          <div className="text-6xl mb-4">🎉</div>
          <h2 className="text-3xl font-bold mb-4" style={{color: '#38bdf8'}}>Quiz Complete!</h2>
          <div className="space-y-2 mb-6">
            <p className="text-lg" style={{color: '#38bdf8'}}>Score: <span className="font-bold">{score}/{totalAnswered}</span></p>
            <p className="text-lg" style={{color: '#38bdf8'}}>Accuracy: <span className="font-bold">{accuracy}%</span></p>
            <p className="text-lg" style={{color: '#38bdf8'}}>Best Streak: <span className="font-bold">{streak}</span></p>
          </div>
  ...
  return (
    <div className="min-h-screen p-8" style={{background: '#111827'}}>
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="rounded-xl shadow-lg p-6 mb-6" style={{border: '2px solid #38bdf8', boxShadow: '0 0 20px rgba(56,189,248,0.3)', background: '#111827'}}>
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl font-bold" style={{color: '#38bdf8'}}>LINUX DRILL</h1>
            <button
              onClick={() => setSelectedCategory(null)}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div className="p-3 rounded-lg" style={{background: '#1f2937', border: '1px solid #38bdf8'}}>
              <div className="text-2xl font-bold" style={{color: '#38bdf8'}}>{score}/{totalAnswered}</div>
              <div className="text-sm" style={{color: '#38bdf8'}}>SCORE</div>
            </div>
            <div className="p-3 rounded-lg" style={{background: '#1f2937', border: '1px solid #38bdf8'}}>
              <div className="text-2xl font-bold" style={{color: '#38bdf8'}}>{accuracy}%</div>
              <div className="text-sm" style={{color: '#38bdf8'}}>ACCURACY</div>
            </div>
            <div className="p-3 rounded-lg" style={{background: '#1f2937', border: '1px solid #38bdf8'}}>
              <div className="text-2xl font-bold" style={{color: '#38bdf8'}}>{streak}</div>
              <div className="text-sm" style={{color: '#38bdf8'}}>STREAK 🔥</div>
            </div>
            <div className="p-3 rounded-lg" style={{background: '#1f2937', border: '1px solid #38bdf8'}}>
              <div className="text-2xl font-bold" style={{color: '#38bdf8'}}>{wrongQueue.length}</div>
              <div className="text-sm" style={{color: '#38bdf8'}}>WRONG QUEUE</div>
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="w-full bg-gray-700 rounded-full h-2" style={{background: '#1f2937', border: '1px solid #38bdf8'}}>
              <div
                className="h-2 rounded-full transition-all duration-300"
                style={{ width: `${((currentQuestionIndex + 1) / currentQuestions.length) * 100}%`, background: '#38bdf8' }}
              ></div>
            </div>
        ...
        {/* Question */}
        <div className="rounded-xl shadow-lg p-8 mb-6" style={{border: '2px solid #38bdf8', boxShadow: '0 0 20px rgba(56,189,248,0.3)', background: '#111827'}}>
          ...
          {!showExplanation ? (
            <div className="space-y-4">
              <input
                ...
                style={{color: '#38bdf8', background: '#1f2937', border: '2px solid #38bdf8', fontFamily: 'monospace', fontSize: '1.125rem'}}
            ...
            <div className="space-y-6">
              <div style={{padding: '1rem', borderRadius: '0.5rem', background: isCorrect ? '#1a3a1a' : '#23303f', border: `2px solid ${isCorrect ? '#38bdf8' : '#ff4444'}`}}>
                ...
                {!isCorrect && (
                  <div className="font-mono text-lg" style={{color: '#38bdf8'}}>
                    Correct answer: <span style={{color: '#38bdf8', fontWeight: 'bold'}}>{currentQuestion.answer}</span>
                  </div>
                )}
              </div>

              <div style={{background: '#111827', padding: '1.5rem', borderRadius: '0.5rem', border: '1px solid #38bdf8', boxShadow: '0 0 10px rgba(56,189,248,0.2)'}}>
                ...
                <p style={{color: '#38bdf8', marginBottom: '1rem', fontStyle: 'italic'}}>{currentQuestion.memoryTip}</p>

                <h4 className="font-semibold mb-2" style={{color: '#38bdf8'}}>💡 Examples</h4>
                <ul className="space-y-1 mb-4">
                  {currentQuestion.examples.map((example, index) => (
                    <li key={index} style={{fontFamily: 'monospace', fontSize: '0.875rem', background: '#1f2937', padding: '0.5rem', borderRadius: '0.25rem', border: '1px solid #38bdf8', color: '#38bdf8'}}>
                      {example}
                    </li>
                  ))}
                </ul>
                ...
                <div style={{background: '#111827', color: '#38bdf8', padding: '1rem', borderRadius: '0.5rem', fontFamily: 'monospace', fontSize: '0.875rem', overflowX: 'auto', border: '1px solid #38bdf8'}}>
